...
<div class="card mb-4">
    <div class="card-header bg-white">
        <h6 class="m-0 font-weight-bold">Form Edit Menu</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.menu.update', $menu->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="tempat_kuliner_id" id="tempat_kuliner_id" class="form-select @error('tempat_kuliner_id') is-invalid @enderror" required>
                    ...
                    @foreach($tempatKuliners as $tempat)
                        <option value="{{ $tempat->id }}" {{ (old('tempat_kuliner_id', $menu->tempat_kuliner_id) == $tempat->id) ? 'selected' : '' }}>
                            {{ $tempat->nama_tempat }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('harga') is-invalid @enderror" id="harga" name="harga" value="{{ old('harga', $menu->harga) }}" required min="0">
                @error('harga')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
