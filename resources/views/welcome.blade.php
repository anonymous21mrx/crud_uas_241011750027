        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($tempatKuliners as $kuliner)
                <div class="bg-white rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-1 transition duration-300 flex flex-col">
                    @if($kuliner->gambar)
                        <img src="{{ (str_starts_with($kuliner->gambar, 'http') || str_starts_with($kuliner->gambar, 'data:image')) ? $kuliner->gambar : Storage::url($kuliner->gambar) }}" alt="{{ $kuliner->nama_tempat }}" class="w-full h-56 object-cover">
                    @else
